...
from galleries.data.models import Galleries1990
...
def search_name(request):
    search_results = Galleries1990.objects.filter(name__icontains=request.POST['name']).order_by('name')
...
def search_zipcode(request):
    search_results = Galleries1990.objects.filter(address_zip__icontains=request.POST['zipcode']).order_by('name')
...
def search_outcome(request):
    search_results = Galleries1990.objects.filter(outcome__icontains=request.POST['outcome']).order_by('name')
...
def search_founded(request):
    ...
    search_results = Galleries1990.objects.filter(start_year__range=(founded_1, founded_2)).order_by('name')
...
def search_closed(request):
    ...
    search_results = Galleries1990.objects.filter(end_year__range=(closed_1, closed_2)).order_by('name')
...
def search_neighborhood(request):
    search_results = Galleries1990.objects.filter(neighborhood__icontains=request.POST['neighborhood']).order_by('name')
...
def detail(request, id):
    try:
         p = Galleries1990.objects.get(pk=id) 
    except Galleries1990.DoesNotExist:
         raise Http404
